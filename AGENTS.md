...
- `src/prompts.js`：AI 提示词模板库（P2），双语（zh 默认 / en）。静态文本抽为 zh/en 两套词典（`TEXT`），所有 builder 接受可选 `lang` 末参——zh 文案与历史逐字一致（回归测试断言依赖）；动态数据（JSON/数值/profile/备注/记忆原文）语言中立、原样透传（用户自填内容不翻译）。`buildMetricGlossary(lang)` 由 src/settings.js 动态生成指标口径说明；`buildProfileSection(profile, lang)` 生成"用户背景与训练目标"段（身份/目标，未配置则不产生该段），四个场景模板 `buildReviewPrompt` / `buildPlanPrompt` / `buildTaperPrompt` / `buildComparePrompt` 均接受可选 profile 参数统一纳入（review 还会在 activity.note 存在时提示 AI 结合备注印证数据），并另接受可选末参 `skills`（专业知识库段，仅服务端注入，CLI 不传保持原样）与 `now`（当前时刻，缺省 `new Date()`，测试可注入固定值）；`buildDateSection(now, lang)` 生成「当前时间」段（dayjs 按 lang 选 zh-cn/en locale 渲染本地日期/星期/时刻/UTC 偏移，附「训练库日期为 UTC 口径」说明；now 非法或缺失返回 null），由 `assemble` 统一插在指标口径段之后、skills 之前——四场景服务端与 CLI 提示词自动携带，server.js 的 chat/follow_up 系统段另行显式注入；`compactSummaryForPrompt(summary, lang)` 在提交前把随时长线性增长的列表压缩（anomalies 按结构化 type 键聚合为 anomalies_summary、segments 保留首尾省略中段、climbs 只留爬升最大段），保证提示词长度与训练时长无关；`thinToWeekly()` 把逐日 form 序列抽稀为逐周点；`buildAgenticSection(lang)` 生成数据查询与计算工具使用指引（仅 Web 端 AI 调用注入，CLI 提示词命令不注入）；`buildChatInstruction(mode, lang)` 生成对话指令（follow_up：≤200 字快答、工具查询不计入字数；chat：务实简洁、结合数据、不限字数）；`buildMemorySection(memories, lang)` 生成「用户记忆」段（逐条带日期与 id、冲突以最新为准规则、save_memory 主动记录指引——无有效记忆时也返回该段显示"暂无"，保证指引始终注入；仅 Web 端 agentic 调用注入，CLI 提示词命令不注入）。纯函数、无 IO。
...
- **数据层语言中立**：summary.json 不存展示文本——anomalies 结构化（type 枚举）、cadence_power.style 枚举、athlete_context.form_state 枚举，展示时按请求语言渲染（formatAnomaly / form.note.* / cad.style.*）；旧库字符串数据原样透传。用户自填内容（身份/目标/备注/记忆）不翻译。
