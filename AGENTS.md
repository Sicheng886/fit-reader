...
- `src/planning.js`：计划推演纯函数（无 IO，仿 src/ftp.js）。`simulateForm({startCtl, startAtl, plan, cfg})` 按 computeForm 同一递推口径推演未来逐日 CTL/ATL/TSB（紧凑数组输出），并按 FORM_SIMULATION 阈值标注 TSB 持续过低与 CTL 周增幅超阈风险；`generateWorkout({target, durationMinutes, ftpWatts, tsb, templates, tsbRecovery})` 按 WORKOUT_TEMPLATES 模板拼装单次课表（热身/主组/冷身，组数由可用时长推算，剩余时长并入冷身保证各段合计=总时长；TSS 估算为逐段 时长秒×IF²/36；TSB 低于阈值自动降级恢复课；时长装不下最小组数返回 error）。
...
- `src/skills.js`：技能文档系统（训练科学知识库注入 AI 提示词）。`loadSkills()` 每次调用实时扫描 `skills/` 目录（不缓存——用户直接往目录放 `.md` 文件即可扩充，无需改代码/重启；文件名排序加载、`_` 前缀与非 `.md` 跳过、空文件与读取失败跳过、目录不存在返回 `[]`；标题取文件首个 `# ` 一级标题行，缺失用文件名兜底；目录可用环境变量 `FIT_SKILLS_DIR` 覆盖，测试隔离用），`buildSkillsSection()` 拼成「专业知识库」提示词段（无技能返回 null）。注入点：server.js 四场景报告（经 prompts.js 四模板的可选 `skills` 末参，插在指标口径段之后）+ chat 直接对话系统段；follow_up 追问与 CLI 提示词命令不注入。内置技能：`skills/01-coggan-power.md`（Coggan 功率体系）、`02-trainingpeaks-load.md`（CTL/ATL/TSB 负荷模型）、`03-heart-rate.md`（心率区间与解读）、`04-report-writing.md`（报告撰写规范）；`skills/_README.md` 为目录使用说明（不注入）。
...
- `server.js`：Web 服务（P4，`npm run web`）。Node 内置 http，零依赖；静态托管 `web/` 前端 + REST API（`/api/overview` 仪表盘聚合（含 `athlete_configured` 首开引导标记）、`GET/POST /api/athlete` 骑手参数查询与更新（写训练库 settings 表并原地生效）、`GET/POST /api/ai-config` AI 服务配置查询与更新（同样写 settings 表并原地生效）、`GET/POST /api/profile` 用户身份与训练目标查询与更新（写 settings 表 profile 行，各场景 AI 提示词纳入考量）、`POST /api/activity/category` 训练分类标记、`POST /api/activity/note` 训练备注保存（空串清除）、`/api/activity`（完整 summary + `zone_ranges` 分区具体范围：POWER_ZONES/HR_ZONES × athlete_context 骑手参数换算的 W/bpm 区间文本，前端分区条右侧显示）、`/api/records` 时序抽稀 ≤1400 点、`POST /api/upload` 原始字节上传 FIT 即分析入库、`GET /api/ftp-estimate` FTP 历史估算、`POST /api/ftp-apply` 把估算 FTP 写入训练库骑手参数并原地更新 `ATHLETE` 使当前进程即时生效、`POST /api/ai` 四场景 AI 报告/提示词（提示词统一注入专业知识库段，见 src/skills.js）、返回 `marked` 渲染后的 HTML、保存到 `ai_reports` 表并保留每 mode 最近 30 条、`POST /api/ai/chat` AI 对话统一入口（follow_up 报告追问 / chat 直接对话；落库 user 消息 + pending 占位 → 202 → 后台 agentic 生成回填，系统段每轮重拼：指令 + 关联报告正文与压缩训练数据 + 工具指引）、`GET /api/ai/chat`（对话详情，有 pending 时 202 快照供轮询）、`GET /api/ai/chats`（对话列表，可选 report_id 找回该报告的追问对话）、`DELETE /api/ai/chat`（整串删除，每 mode 滚动保留 50 个）、`GET /api/ai/reports`、`GET /api/ai/report`、`GET /api/ai/memories`（全部记忆含已取代）、`DELETE /api/ai/memory`）；报告与对话的 agentic 调用注入记忆段（最近 30 条有效记忆），save_memory 的 source 场景标记由调用方透传；文件名参数一律 basename 防路径穿越；输出/输入目录可用 `FIT_OUTPUT_DIR` / `FIT_INPUT_DIR` 覆盖（测试隔离）。
- `web/`：纯 HTML/CSS/JS SPA（P4），零依赖零构建，浏览器原生 ES Module（`index.html` 以 `<script type="module">` 加载 `app.js`，server.js 已按 `text/javascript` 提供 `.js`）：模块化结构为 `web/app.js`（入口：hash 路由 + 顶栏导航 + 首开引导）、`web/js/common.js`（共享基础：$/esc/api/弹窗/格式化/徽章标签常量/全局 state 与概览缓存/顶栏骑手参数条/训练列表行）、`web/js/charts.js`（手写 SVG 图表：多系列时序图各系列独立纵轴、≤180s 短缺口线性插值桥接显示（码表自动暂停产生的停顿不断线，仅显示层不改数据）、CTL/ATL/TSB 趋势、分区分布、峰功率曲线）、`web/js/ai.js`（AI 报告生成/历史列表/追问/对话轮询与消息渲染——详情页、AI 报告页、对话页三方共用）、`web/js/views/`（8 个视图各一文件：dashboard/activities（含详情）/upload/ai/chat/settings/memory）；依赖方向单向：views → common/charts/ai，app.js → views，禁止视图互相 import。路由覆盖概览/训练/详情/上传/AI 报告/对话/记忆/设置，导航栏右侧三点菜单展开「记忆」与「设置」；详情页含训练分类标记与训练备注（体感/路况自由注释，保存按钮写库，AI 复盘纳入考量）；「AI 报告」页生成/查看报告并支持追问（持久化 follow_up 对话：提交后异步生成、前端轮询、可关闭页面事后回看，回答 ≤200 字且结合本次训练具体数据，报告下可「清除追问」）；「对话」页（#/chat）支持无报告上下文的直接对话（AI 经 agentic 工具自行取数）、对话新建/删除与 pending 轮询；「记忆」页（#/memory）独立列出全部 AI 记忆含已取代标记，只做删除不做编辑；设置页维护骑手参数、身份与训练目标（身份一句话/目标一段话，AI 报告纳入考量）与 AI 服务配置（存训练库，保存后跳回首页，未配置时首开自动引导）；暗色平面科技运动风（荧光黄 volt 主色 + 斜切元素 + 等宽斜体大数字）。
...
- `test/skills.test.mjs`：src/skills.js 技能文档加载——临时目录 + `FIT_SKILLS_DIR` 隔离（loadSkills 每次调用实时读环境变量，无需 import 前设置），覆盖排序/`_` 前缀与非 .md 跳过/标题提取与文件名兜底/目录不存在/拼装格式/实时生效/内置四技能冒烟。
