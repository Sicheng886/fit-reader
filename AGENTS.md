...
- `src/prompts.js`：AI 提示词模板库（P2），双语（zh 默认 / en）。静态文本抽为 zh/en 两套词典（`TEXT`），所有 builder 接受可选 `lang` 末参——zh 文案与历史逐字一致（回归测试断言依赖）；动态数据（JSON/数值/profile/备注/记忆原文）语言中立、原样透传（用户自填内容不翻译）。`buildMetricGlossary(lang)` 由 src/settings.js 动态生成指标口径说明；`buildProfileSection(profile, lang)` 生成"用户背景与训练目标"段（身份/目标，未配置则不产生该段），四个场景模板 `buildReviewPrompt` / `buildPlanPrompt` / `buildTaperPrompt` / `buildComparePrompt` 均接受可选 profile 参数统一纳入（review 还会在 activity.note 存在时提示 AI 结合备注印证数据），并另接受可选末参 `skills`（专业知识库段，仅服务端注入，CLI 不传保持原样）；`compactSummaryForPrompt(summary, lang)` 在提交前把随时长线性增长的列表压缩（anomalies 按结构化 type 键聚合为 anomalies_summary、segments 保留首尾省略中段、climbs 只留爬升最大段），保证提示词长度与训练时长无关；`thinToWeekly()` 把逐日 form 序列抽稀为逐周点；`buildAgenticSection(lang)` 生成数据查询与计算工具使用指引（仅 Web 端 AI 调用注入，CLI 提示词命令不注入）；`buildChatInstruction(mode, lang)` 生成对话指令（follow_up：≤200 字快答、工具查询不计入字数；chat：务实简洁、结合数据、不限字数）；`buildMemorySection(memories, lang)` 生成「用户记忆」段（逐条带日期与 id、冲突以最新为准规则、save_memory 主动记录指引——无有效记忆时也返回该段显示"暂无"，保证指引始终注入；仅 Web 端 agentic 调用注入，CLI 提示词命令不注入）。纯函数、无 IO。
...
- 当前版本：`1.6.0`。版本号遵循语义化版本（SemVer）：`MAJOR.MINOR.PATCH`，bug 修复/小调整升 PATCH，新增功能升 MINOR，破坏性改动升 MAJOR。
- 推荐工作流：
  1. 完成修改并跑通 `npm test`。
...
- **AI 报告语言**：提示词整体按语言生成（prompts.js 双语词典，英文模板自然产出英文报告，无需额外语言指令）；专业知识库按语言加载（skills/ 的 `.en.md` 文件）；工具 schema（tools.js `toolDefs(lang)`）与工具错误消息随语言切换。历史报告 markdown 保持生成时语言，不事后翻译。
...
- `test/unit.test.mjs`：指标算法纯函数单测（NP/peakAvg/分区/爬升去抖/间歇识别/心率漂移/FTP 估算/缺失检测/开发者字段 + src/ftp.js 历史估算：CP 模型/数据充分性/心率交叉验证各分支 + src/prompts.js 提示词数据压缩 + src/planning.js：simulateForm 衰减/收敛/两类风险触发与不触发、generateWorkout 五种模板结构/瓦数换算/TSS 口径/时长不足报错/TSB 降级）。
- `test/skills.test.mjs`：src/skills.js 技能文档加载——临时目录 + `FIT_SKILLS_DIR` 隔离（loadSkills 每次调用实时读环境变量，无需 import 前设置），覆盖排序/`_` 前缀与非 .md 跳过/标题提取与文件名兜底/目录不存在/拼装格式/实时生效/内置四技能冒烟/**`.en.md` 按语言加载与 Knowledge Base 段**。
