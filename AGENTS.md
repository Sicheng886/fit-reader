...
- `ai.js`：AI API 客户端（P4）。OpenAI 兼容 chat/completions（Node 内置 fetch，零依赖）；环境变量 `FIT_AI_API_KEY`（必填）/ `FIT_AI_BASE_URL`（默认 Kimi `https://api.moonshot.cn/v1`）/ `FIT_AI_MODEL`（默认 `moonshot-v1-32k`）/ `FIT_AI_TEMPERATURE`（可选，缺省不传）/ `FIT_AI_TIMEOUT_MS`（默认 5 分钟，复盘建议 10 分钟起步）；配置项调用时惰性读取，`.env` 由 server.js 入口通过 Node 内置 `process.loadEnvFile()` 注入（仅入口分支加载，测试 import 时不触发）；`isAiConfigured()` 供调用方判断退化为复制提示词模式。
