...
- `web/`：纯 HTML/CSS/JS SPA（P4），零依赖零构建：hash 路由（概览/训练/详情/上传/AI 分析/对话/设置）、手写 SVG 图表（多系列时序图各系列独立纵轴、≤180s 短缺口线性插值桥接显示（码表自动暂停产生的停顿不断线，仅显示层不改数据）、CTL/ATL/TSB 趋势、分区分布、峰功率曲线）；详情页含训练分类标记与训练备注（体感/路况自由注释，保存按钮写库，AI 复盘纳入考量）；设置页维护骑手参数、身份与训练目标（身份一句话/目标一段话，AI 报告纳入考量）、AI 服务配置（存训练库，保存后跳回首页，未配置时首开自动引导）与「AI 记忆」管理（列出全部记忆含已取代标记，只做删除不做编辑——错了就删，AI 会重记）；AI 报告使用服务端 `marked` 渲染后的 HTML，并支持历史报告列表（每 mode 最近 30 条）与追问（持久化 follow_up 对话：提交后异步生成、前端轮询、可关闭页面事后回看，回答 ≤200 字且结合本次训练具体数据）；「对话」页（#/chat）支持无报告上下文的直接对话（AI 经 agentic 工具自行取数）、对话新建/删除与 pending 轮询；暗色平面科技运动风（荧光黄 volt 主色 + 斜切元素 + 等宽斜体大数字）。
