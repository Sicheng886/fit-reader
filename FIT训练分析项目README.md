...
- **概览**：CTL/ATL/TSB 大数字仪表 + 最近 90 天负荷趋势图 + 月度汇总 + 最近训练；FTP 卡片上的「科学估算 FTP」按钮基于最近 6 周骑行的功率峰曲线 + 心率交叉验证估算 FTP（双参数 CP 模型 + Coggan 20min×0.95 互校），数据不足/漂移时给出需要收集的数据清单，高置信时可一键写回 `settings.js`
...
- FTP 历史估算（`ftp.js`，Web 仪表盘「科学估算 FTP」）：取最近 6 周（可配）全部骑行的 5min/20min 峰功率，双方法互校——① Morton 双参数临界功率模型 P(t)=CP+W′/t 解出 CP≈FTP；② Coggan 20min 峰功率×0.95；心率交叉验证（锚点骑行心率峰值未达 90% HRmax 判定非全力、功率/心率区间系统性偏移提示 FTP 配置漂移、心率漂移中位数 >5% 提示疲劳/脱水数据漂移）；数据不足时输出需要收集的数据清单（如"充分休息后的 20 分钟全力测试，佩戴心率带"）；高置信时可一键写回 settings.js 并进程内即时生效
...
| `settings.js`          | 全部可调配置：骑手参数、分区定义、各分析算法阈值（含 `FTP_ESTIMATION`） |
